...
const excludedRootNames = new Set([
    ...
    'auth.json',
    /*
     * 'deploy' คือเครื่องมือกับเอกสารของฝั่งเรา ไม่ใช่โค้ดที่รันบนเซิร์ฟเวอร์
     *
     * สองเหตุผลที่ต้องกันออก:
     *   1. deploy/public_html คือชุดที่แตกไว้ให้ลากขึ้น DirectAdmin ถ้าไม่กันออก
     *      การสร้างชุดครั้งถัดไปจะห่อสำเนาของตัวเองซ้อนเข้าไปอีกชั้น
     *   2. ไฟล์ .sql ในนี้บอกโครงสร้างฐานข้อมูลทั้งหมด และ .htaccess ของเซิร์ฟเวอร์
     *      ปิดไว้แค่ app/ config/ vendor/ .env — โฟลเดอร์นี้จึงเปิดผ่านเว็บได้ถ้าอัปขึ้นไป
     */
    'deploy',
    'deploy-output',
    'node_modules',
