@php
    ...
    $meetingShowParams = array_filter(['meeting' => $meeting->id, 'employee' => $inspectedEmployee?->id, 'from' => $meetingFrom ?? null]);
@endphp
<article class="meetings-page__card">
    ...
    <div class="meetings-page__card-body">
        <div class="meetings-page__card-title"><div><span class="meetings-page__status meetings-page__status--{{ $status[1] }}">{{ $status[0] }}</span>@if($inspectedRole)<span class="meetings-page__role">{{ $inspectedRole }}</span>@endif<h2><a href="{{ route('meetings.show', $meetingShowParams) }}">{{ $meeting->title }}</a></h2></div></div>
    ...
    <div class="meetings-page__card-actions">
        <a class="meetings-page__icon-button" href="{{ route('meetings.show', $meetingShowParams) }}" aria-label="ดูรายละเอียด {{ $meeting->title }}"><i class="bi bi-eye" aria-hidden="true"></i></a>
        @can('update', $meeting)<a class="meetings-page__icon-button" href="{{ route('meetings.show', $meetingShowParams + ['edit' => 1]) }}" aria-label="แก้ไข {{ $meeting->title }}"><i class="bi bi-pencil" aria-hidden="true"></i></a>@endcan
