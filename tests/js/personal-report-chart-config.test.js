...
test('priority config renders a neutral empty state and five real values', () => {
    const empty = priorityChartConfig({ labels: [], values: [] });
    assert.deepEqual(empty.data.datasets[0].data, [1]);
    assert.equal(empty.options.plugins.tooltip.enabled, false);
